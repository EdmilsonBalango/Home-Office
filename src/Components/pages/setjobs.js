...
export default function SetJobs() {
    ...
    async function submit(event) {
        ...
        const request = {
            jobdetail: document.getElementById("detailjob").value,
            category: document.getElementById("category").value,
            requires: document.getElementById("requires").value,
            author: localStorage.getItem('auth')
        }
        ...
        await api.post(`/fluxos/jobs`, request).then(Response => {
    ...
    return (
        <Container>
            <Boxform onSubmit={submit}>
                <InputBoxs type="text" placeholder={"detalhes do job"} id={'detailjob'}></InputBoxs>
                <select id={'category'}>
                    {Categories.map(cat => {
                        return (
                ...
                <SubmitButton type={"submit"} value={"Postar job"} />
            </Boxform>
        </Container>
    );
}